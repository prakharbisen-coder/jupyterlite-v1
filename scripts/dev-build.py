...
def build():
    ...
    try:
        ...
        patch_script = ROOT / "scripts" / "patch_kernel.py"
        if patch_script.exists():
            subprocess.run([sys.executable, str(patch_script)], cwd=ROOT, check=True)
        else:
            print(f"[warn] Kernel patch script not found: {patch_script}")
